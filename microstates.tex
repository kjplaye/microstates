...
the total linear momentum vanishes, $k + (-k) = 0$; the anticorrelated 
helicities $(\lambda_R, \lambda_L) = (+1,-1)$ and $(-1,+1)$ each 
carry zero net angular momentum, and the Bell state 
superposition~\eqref{eq:microstate_vec} preserves this, so the source 
exhibits no net back-reaction in linear or angular momentum.
...
unique state in this momentum sector invariant under that 
...
between wedges.
The Bell state~\eqref{eq:microstate_vec} is the unique two-photon 
state at fixed frequency $\omega$ and fixed momentum pair 
$(k, -k)$ invariant under $J$: within this momentum sector, 
...
momentum conservation at the emission vertex. The argument 
...
\begin{aligned}
    ...
    &= \mathscr{L}_{\text{free}}
      ...
      \;+\; \text{h.c.}
\end{aligned}
\label{eq:bilocal_lag_field}
\end{equation}
Substituting the mode expansions $A^+_i(x) = \int 
\frac{d^3k}{(2\pi)^3} \frac{1}{\sqrt{2\omega_k}} 
\sum_\lambda \epsilon^i_{(\lambda)} a^{R}_{k,\lambda} 
u^R_\omega(x) + (L \leftrightarrow R)$ and using the 
orthogonality of the circular polarization vectors 
$\epsilon^{i*}_{(\lambda)}\epsilon^i_{(\lambda')} = 
\delta_{\lambda\lambda'}$, the interaction term reduces to
\begin{equation}
\begin{aligned}
    \mathscr{L}_{\text{sourced}}
    &= \mathscr{L}_{\text{free}}
...
Since the bilocal source need not account for the 
entire acceleration of $\mathcal{R}$, other interactions 
may contribute, $\dot{N}$ is strictly a lower bound on 
the pair production rate required to sustain the acceleration; 
the Landauer consistency condition applies to the bilocal 
contribution alone.
...
the bilocal source, via the PVM projection of 
...
\end{equation}
The $\mathcal{O}(g^2)$ corrections are multi-photon terms 
arising from the two-mode squeezing 
interaction~\eqref{eq:bilocal_lag}: at order $g^2$ the 
state includes two-pair contributions 
$|2_{\omega,+1}\rangle_R|2_{\omega,-1}\rangle_L$ and 
their permutations, suppressed relative to the single-pair 
term by $g^2/\omega^2$ in natural units. The single-microstate 
description is therefore valid in the weak-coupling regime 
$g \ll \omega$, which we assume throughout.
...
PVM limit of the Kraus family~\eqref{eq:kraus_longitudinal}: 
