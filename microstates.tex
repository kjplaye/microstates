...
the geometry of the bulk ER bridge. Here the 
direction of explanation is reversed: the emission 
event at $\mathcal{P}$ is primary, and the geometry,
the AS pp-wave, the horizon area increment, the 
BMS supertranslation, is the output of the 
subsequent measurement events at $\mathcal{R}$ 
and $\mathcal{L}$. No holographic 
dual is needed because the geometry is not the 
starting point but the result. Thermality is not an 
objective feature of the vacuum that geometry imposes 
on accelerating observers; it is an epistemic 
consequence of ignorance about $\mathcal{P}$, and the 
non-thermal microstate is what remains when that 
...
\end{equation}
where $u = t - z + 1$ is the null coordinate\footnote{In 
\cite{player2025driving}, which works in 1+1 
dimensions with spatial coordinate $x$, the 
complex Stokes variable is denoted $z = iq\sigma 
+ \mu/\sigma$; we use $\zeta$ here to avoid 
confusion with the longitudinal spatial coordinate 
$z$ of the present paper.} 
defined in Section~\ref{sec:ppwave}.
The operator $K_\sigma$ together with its complement 
$K_\sigma^\perp$ satisfying 
$K_\sigma^\dagger K_\sigma + 
(K_\sigma^\perp)^\dagger K_\sigma^\perp = \mathbf{1}$ 
defines a one-parameter POVM family; 
...
traces to the same algebraic symmetry.
The Bell state~\eqref{eq:microstate_vec} is a 
two-photon state invariant under the 
