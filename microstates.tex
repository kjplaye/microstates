...
Newton's second law alone. We develop an information-theoretic 
language of \emph{handoff events}, \emph{matter-memory}, and 
\emph{spatial bits}, in which matter and horizon are complementary
timelike and spacelike channels. Non-thermal microstates 
constructed as helicity-anticorrelated Bell states each produce 
an Aichelburg--Sexl pp-wave at the Rindler horizon; the 
Raychaudhuri equation yields the event-by-event equality 
$S_{GHY}|_{\mathcal{H}} = S_{\mathrm{matter}}$, with $\ell_P^2$ 
as the conversion factor between geometric and informational 
counts. The Carnot efficiency bound and the Unruh--Landauer 
bound are shown to be the same inequality viewed from opposite 
directions, and the GHY term arises as the effective action for 
one Rindler wedge after integrating out the other.}
...
We argue the answer is quantum measurement. Each act of information 
acquisition by a physical observer constitutes a discrete microstate event; 
its energy--momentum transfer produces gravitational effects as seen by that 
observer. Local quantum field theory near a black hole horizon is identical 
to that of a uniformly accelerated observer in flat spacetime, so the Unruh 
and Hawking effects are the same computation in different 
coordinates~\cite{unruh1976notes, hawking1975particle}, and it is at this 
shared locus that the identification is made precise.

Three existing programmes arrive at closely related conclusions by different 
...
explicit source. The BMS programme~\cite{carlip2018black, hawking2016soft, 
...
with these frameworks but describe the elementary process they collectively 
require.

The central technical result is the event-by-event equality 
...
The identification of quantum measurement events as gravitational 
microstates is a conceptual proposal, not a derivation from deeper 
axioms. We do not claim that measurement events \emph{must} be the 
microstates --- that direction would require a complete theory of 
multi-observer quantum mechanics in curved spacetime, which does not 
yet exist. What we claim is weaker: the identification \emph{can} work. 
The Unruh--Landauer relation is consistent with standard black hole 
thermodynamics; the bilocal microstate construction is internally 
consistent; and the event-by-event equality 
$S_{GHY}|_{\mathcal{H}} = S_{\mathrm{matter}}$ closes the consistency 
loop. Whether the identification is \emph{necessary} is left open.
...
primitive, and individual microstates are non-thermal. The approach 
is also a refactored version of the Di\'osi--Penrose objective collapse 
programme~\cite{penrose1996gravity, diosi1987universal}: where that 
programme postulates that gravitational self-interaction objectively 
collapses the wavefunction, we treat measurement as a subjective process 
mediated by energy--momentum transfer, with state reduction as a Bayesian 
update for a given observer; the contrast is developed in 
...
entanglement-generated geometry, the equivalence of the Carnot and 
