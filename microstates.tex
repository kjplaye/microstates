...
present construction can be read as a microstate-level realisation of the Bousso bound, with each communication contributing one Planck-area patch to the light-sheet.
...
Thermality is not a primitive property of the vacuum but a consequence 
of epistemic coarse-graining: the global Minkowski state $|0\rangle_M$ 
is pure and fixed throughout, and what changes as the source-detector 
pair $(\mathcal{P}, \mathcal{R})$ is specified is the resolution of the 
observer's ignorance about which null geodesic carried the quantum. 
...
$\sigma_\perp \sim c/\omega$ is localised within one wavelength of 
the photon axis. These two limits correspond to geometrically 
distinct pp-waves at the Rindler horizon.
...
($\Delta A = 8\pi G\hbar\omega$ equation (\ref{eq:deltaA_result}) upcoming).
...
The matter-substrate language of this section makes contact 
...
discrete sequence of communication units, each acting as a PVM 
...
thermodynamic limit of discrete Bremermann samples.
...
matter substrate and the communication units are the microscopic 
...
projections, communication units, matter-memory, is the 
natural language in which that problem must be posed when 
the observers are causally separated by a horizon.

Finally, we mention that recent experimental demonstrations
of the Compton clock~\cite{lan2013clock} 
ground the matter-as-clock picture in laboratory physics.
