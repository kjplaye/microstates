...
% TODO 2.3 [NEW]: Clarify the thermality-from-coarse-graining
% claim.
% Section 3 states that thermality is "not a property of any
% single microstate but of the ensemble," arising when one
% discards the specific microstate identity. However, tracing out
% one wedge from the bilocal state at fixed $\omega$ — a single
% microstate — already yields a thermal density matrix, because
% the thermality arises from the entanglement structure of the
% state rather than from averaging over $\omega$. The
% coarse-graining over helicity outcomes is a separate,
% additional step. Distinguish these two sources of thermality
% explicitly: the entanglement-induced thermality at fixed
% $\omega$, and the Planck distribution over $\omega$ which is
% genuinely an ensemble property. Restrict the claim to the
% latter, or restate it to cover both.
