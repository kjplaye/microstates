...
This section is the conceptual core of the paper. We 
propose that matter is the fundamental substrate of 
...
factor between the two descriptions. This is a specific, 
tangible alternative to approaches that quantize geometry 
directly~\cite{wheeler1955geons, bombelli1987space}: 
discreteness lives in the matter sector via $\hbar$, is 
Lorentz invariant by construction, and geometry inherits 
it through the covariant Raychaudhuri equation. No 
statistical recovery of Lorentz invariance is required.

The framework has three interlocking components.

The first (Sections~\ref{sec:channel}--\ref{sec:horizon_channel}) 
establishes matter as a timelike information channel: a 
massive particle carries phase at the Compton frequency 
$mc^2/h$, simultaneously setting its clock rate, its 
sampling rate via Bremermann's limit, and its maximum 
information throughput. The horizon enters as the surface 
at which this timelike channel hands off to the spacelike 
horizon channel, with the Planck area $\ell_P^2$ as the 
conversion factor. This is the information-theoretic 
content of the Bekenstein--Hawking area law: one Planck 
area per handoff event, one handoff event per bit.

The second (Sections~\ref{sec:handoff}--\ref{sec:saturation}) 
makes this precise: the handoff event is defined as the 
transfer of one quantum of action $\hbar$ and one 
classical bit across the horizon. Channel capacity 
saturates at the Rindler horizon at exactly one bit per 
Rindler period, connecting the information-theoretic 
saturation to the PVM limit of 
Section~\ref{sec:pvm_localization} and the EFT boundary 
of Section~\ref{sec:eft}. The consistency check 
$S_{GHY}|_{\mathcal{H}} = S_{\mathrm{matter}}$ of 
Section~\ref{sec:ppwave} is the geometric expression 
of this saturation condition: Section~\ref{sec:ppwave} 
uses the handoff definition and saturation condition 
from this framework and finds them consistent with the 
Raychaudhuri equation event by event.

The third (Sections~\ref{sec:types}--\ref{sec:zigzag}) 
shows that this framework is already implicit in the 
Standard Model: the mass/massless divide is the 
operational distinction between memory substrates and 
handoff carriers, and the Penrose zig-zag construction 
for the Dirac equation is the timelike version of the 
horizon handoff, with the Higgs field playing the role 
of the bilocal source. The bilocal Bell state and the 
Dirac 4-spinor are the same PCT-invariant structure at 
different codimension. This consilience with existing 
physics is not a derivation but a consistency: the 
framework does not contradict the Standard Model but 
shows that the handoff structure is already present in 
it.

The connection to existing programs in quantum gravity 
and quantum foundations is developed throughout. The 
Page--Wootters mechanism~\cite{page1983evolution, 
giovannetti2015quantum} and the Connes--Rovelli modular 
flow~\cite{connes1994neumann} both support the view that 
proper time is emergent; the present framework gives a 
specific physical mechanism. Verlinde's entropic 
gravity~\cite{entropic} operates at the thermodynamic 
level; the present framework supplies the microstate 
structure beneath it. The Wigner's friend 
problem~\cite{brukner2018no} and the 
Frauchiger--Renner theorem~\cite{frauchiger2018quantum} 
identify the multi-observer problem as sharp; the 
information-theoretic vocabulary introduced here,
spatial bits, PVM projections, handoff events, 
matter-memory, is the natural language in which 
that problem must be posed in a gravitational context.
