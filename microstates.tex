...
The combination of Unruh and Landauer as an energy scale 
in quantum gravity appears contemporaneously and 
independently in Fabriciuss~\cite{fabriciuss2025digital}, 
where it calibrates a cosmological horizon tile model 
aimed at the microscopic origin of inertia; that work 
shares the motivation of grounding horizon thermodynamics 
in a microstate layer, but postulates a classical tile 
structure rather than constructing microstates from QFT.
...
only on acceleration; the bound is applied to 
individual carrier quanta rather than to bulk evaporation 
or cosmological averages; and the microstates are 
constructed explicitly as entangled photon pairs in 
Minkowski QFT rather than postulated as discrete 
geometric units.
...
individual photon but at the level of the source rate\footnote{Here we discuss a constant rate with a fixed frequency, but many situations are possible.}. The 
acceleration $a$ does not fix the frequency $\omega$ of each 
selected quantum; rather, it determines the rate
at which the bilocal source selects entangled pairs in order to 
...
\begin{equation}
    a = \frac{\dot{N}(a)\,\hbar\omega}{m_{\mathcal{R}}c},
    \qquad
    \dot{N}(a) = \frac{m_{\mathcal{R}}ca}{\hbar\omega}.
\label{eq:newton_rate}
\end{equation}
This is exact for $\mathcal{R}$ at rest, with no 
causal diamond heuristics required. 
