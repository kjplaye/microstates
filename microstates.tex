...
Thermodynamic approaches to gravity are remarkably successful and remarkably 
incomplete. Jacobson derives the Einstein equations from entropy and temperature 
on local Rindler horizons~\cite{jacobson1995thermodynamics}; Verlinde derives 
gravitational forces from entropy gradients on holographic 
screens~\cite{entropic}. Both frameworks operate at the level of bulk 
thermodynamics and neither specifies what physical process underlies each 
individual entropy increment. This paper proposes an answer: quantum measurement 
events, modeled as energy-momentum transfers under proper acceleration, provide 
explicit realizations of those increments in flat Minkowski space.
...
indistinguishable from acceleration in a gravitational field. The logical 
structure is summarised in Figure~\ref{fig:diagram}.
...
That a single starting point, quantum measurement as energy--momentum transfer 
under proper acceleration, makes simultaneous contact with three independent 
programs, without additional assumptions, suggests that measurement events may 
describe the elementary process those frameworks collectively require.
...
and individual microstates are non-thermal. Our approach is a refactored version 
of the Di\'osi--Penrose objective collapse 
program~\cite{penrose1996gravity, diosi1987universal}: where that program 
postulates that gravitational self-interaction objectively collapses the 
wavefunction, we treat measurement as a subjective process mediated by 
energy--momentum transfer, with state reduction as a Bayesian update for a 
given observer, as illustrated in Figure~\ref{fig:diagram}.
...
approximation happens to give the right answer\footnote{The correspondence is specific to the spherically symmetric case: the 
Newtonian inverse-square law with a point mass reproduces the Schwarzschild 
geometry at the horizon scale, but other matter distributions, the bilocal 
source of Section~\ref{sec:microstates} among them, or the full Einstein 
equations, will in general give different results. The heuristics of this 
section should be read as recovering the Schwarzschild case only; the 
construction layer makes no use of them.} at the 
...
The arguments of Section~\ref{sec:heuristics} establish a 
...
frequency $\omega = \omega_k$. The momenta are perfectly 
balanced, so the source exhibits no net back-reaction in this 
minimal model. The two photons propagate into the left and 
right Rindler wedges, where $\mathcal{L}$ and $\mathcal{R}$ 
detect them respectively, each registering a helicity 
eigenvalue and acquiring one classical bit of information.
...
Rindler wedge. The two null directions of propagation define 
...
\subsection{PVM Localization and the Elimination of Coarse-Graining NEW}
\label{sec:pvm_localization}

The Unruh mode plays a double role in this construction. 
As a QFT object it is a coefficient in a mode expansion, 
...
that the present construction eliminates explicitly.
...
where the $e^{-\frac{1}{4}z^2}$ branch dominates, 
and $\sigma \to 0$ gives the localized non-thermal 
regime where the $e^{+\frac{1}{4}z^2}$ branch dominates, 
with the Stokes line at $\arg z = \pi/4$ marking the 
...
\end{equation}
where $u = t - z + 1$,
parametrizing a one-parameter POVM family in the 
longitudinal direction. Specifying $\mathcal{R} = (0,1)$ 
...
possible source-detector pairs $(\mathcal{P}', 
\mathcal{R}')$ connected by null geodesics at that 
frequency, uniform in the transverse plane. Specifying 
...
free parameter in Section~\ref{sec:microstates} and 
...
reflect the same epistemic situation. Specifying the physical 
locations of $\mathcal{P}$ and $\mathcal{R}$ (and $\mathcal{L}$ respectively) breaks this 
ignorance explicitly, eliminating both coarse-grainings 
responsible for the apparent thermality of the standard Unruh 
effect~\cite{player2025driving}: the unknown frequency of the 
detected excitation, and the unspecified transverse position 
of the source-detector pair. We now show that both 
coarse-grainings are resolved by a single projective 
argument, and that the interpolation between them is 
controlled by a one-parameter family of diagonal Kraus 
operators.

\paragraph{Frequency localization.}
In~\cite{player2025driving} the inversion of the standard 
Unruh logic rests on a PVM projection in frequency space: 
specifying the detector location $\mathcal{R}$ selects the 
Rindler frequency $\omega$ the detector responds to, rather 
than having the thermal bath impose a distribution over all 
frequencies. Because $\mathcal{P}$ emits at a definite 
frequency $\omega$,
a PVM projection onto the definite Rindler frequency $\omega$ 
emitted by $\mathcal{P}$ resolves the frequency coarse-graining: 
after the collapse there is no $\omega$-averaging and no 
partial-trace ambiguity, and the observer is at a specific 
point in microstate space within the non-thermal Minkowski 
description.
In this sense measurement is access to the 
Minkowski description, and the apparent thermal bath is 
replaced by a definite momentum transfer, a thrust, sourced 
by $\mathcal{P}$. This reinterprets a portion of the Unruh 
response as physically sourced thrust rather than thermal 
...
Figure~\ref{paz_freq} illustrates the situation: the Unruh mode 
phase structure connects $\mathcal{P}$ and $\mathcal{R}$ 
via the null geodesic, and the PVM projection onto this 
geodesic selects a single non-thermal microstate from 
the thermal superposition.
...
The rainbow coloring shows the Unruh mode phase structure 
across Minkowski space, analytically continued from the 
right Rindler wedge. A PVM projection onto this null 
geodesic selects a single frequency from the thermal 
superposition, replacing the full delocalized mode with 
the localized non-thermal microstate of 
equation~\eqref{eq:microstate_vec}.}
\label{paz_freq}
\end{figure}

\paragraph{Transverse localization.}

The same projective logic extends to the transverse 
directions. The thermofield double state~\eqref{eq:vacuum_vec} 
is a sum over all Rindler frequencies $\omega$ and all 
transverse positions $\mathbf{x}_\perp$. A Rindler plane wave 
at frequency $\omega$ is a coherent superposition over all 
possible source-detector pairs $(\mathcal{P}', \mathcal{R}')$ 
connected by null geodesics at that frequency:
%
\begin{equation}
    |1_{\omega,\mathrm{plane}}\rangle 
    = \int d^2\mathbf{x}_\perp\, d\eta'\;
      \phi(\mathbf{x}_\perp, \eta')\;
      |\mathcal{P}'(\mathbf{x}_\perp,\eta'),\,
       \mathcal{R}'(\mathbf{x}_\perp,\eta')\rangle,
    \label{eq:plane_superposition}
\end{equation}
%
with flat weight $\phi$. The decomposition is schematic, 
representing the intuition that a plane wave 
is a uniform superposition over all possible 
source-detector geometries at frequency $\omega$, 
rather than a literal Fock space expansion.

Specifying $\mathcal{P} = (-1,0)$ 
and $\mathcal{R} = (0,1)$ projects onto the unique null 
geodesic connecting them, resolving the transverse 
coarse-graining simultaneously with the frequency 
coarse-graining. A single PVM projection onto this geodesic 
therefore selects a single term from the thermofield double 
sum in both directions at once.

ATTEMPT TO CLEAN THINGS UP BELOW

A Unruh mode at frequency $\omega$ can be understood 
as a uniform superposition over all possible source-detector 
pairs $(\mathcal{P}', \mathcal{R}')$ connected by null 
geodesics at that frequency, with flat weight over their 
transverse positions $\mathbf{x}_\perp$ and Minkowski time 
$t'$. This decomposition is at the level of mode functions; 
the corresponding quantum state is the thermofield double 
sum~\eqref{eq:vacuum_vec}, which is the Fock space 
realisation of the same superposition. Specifying 
$\mathcal{P} = (-1,0)$ and $\mathcal{R} = (0,1)$ then 
performs a PVM projection at the state level, selecting 
the unique term corresponding to the 
$\mathcal{P}$--$\mathcal{R}$ null geodesic and yielding 
the localized non-thermal 
microstate~\eqref{eq:microstate_vec}.

A single PVM projection onto the $\mathcal{P}$--$\mathcal{R}$ 
null geodesic therefore resolves both coarse-grainings 
simultaneously: the frequency coarse-graining of the 
paragraph above and the transverse coarse-graining 
addressed here. One physical specification of source 
and detector does the work of two separate projections.

\paragraph{From POVM to PVM: the Stokes transition.}
The sharpness of the transverse projection is controlled by 
the width $\sigma$ of the wavepacket shaping function, which 
acts as a diagonal Kraus operator:
%
\begin{equation}
...
\end{equation}
%
with complementary operator $K_\sigma^\perp$ satisfying
$K_\sigma^\dagger K_\sigma + (K_\sigma^\perp)^\dagger K_\sigma^\perp = \mathbf{1}$ 
pointwise~\cite{kraus1983states, nielsen2001quantum}, since $e^{-r^2/\sigma^2} + 
(1 - e^{-r^2/\sigma^2}) = 1$. This defines a one-parameter 
POVM family interpolating between two limits: $\sigma \to 
\infty$ gives $K_\sigma \to \mathbf{1}$, no projection, 
recovering the full thermal plane wave; $\sigma \to 0$ gives, after normalisation by 
$(\pi\sigma^2)^{-1}$, a sharp PVM projector 
onto the $\mathcal{P}$--$\mathcal{R}$ null 
geodesic, recovering the localized non-thermal 
microstate. The normalisation is required 
because $\int e^{-r^2/\sigma^2}\,d^2x_\perp 
= \pi\sigma^2 \to 0$ as $\sigma \to 0$; the 
PVM limit is approached by the normalised 
operator $(\pi\sigma^2)^{-1} K_\sigma$, which 
converges to the projector 
$|\mathbf{x}_\perp = 0\rangle
\langle\mathbf{x}_\perp = 0|$ 
onto the beam axis.
...
\captionsetup{width=0.7\textwidth}
\caption{Trajectory of $z = iq\sigma + \mu/\sigma$ 
in the complex plane as $\sigma$ interpolates 
between the two limits of the Kraus 
family~\eqref{eq:kraus}. The thermal limit 
$\sigma \to \infty$ corresponds to $z$ on the 
positive imaginary axis; the localized PVM limit 
$\sigma \to 0$ corresponds to $z$ on the positive 
real axis. The green Stokes lines at 
$\arg z = \pi/4$ mark the boundary between the 
two asymptotic regimes of the parabolic cylinder 
function $D_\nu(-z)$, and therefore the boundary 
between the POVM and PVM descriptions of the 
transverse localization. Figure 
from~\cite{player2025driving}.}
\label{stokes_kraus}
\end{figure}


The Stokes phenomenon of~\cite{player2025driving}, Figure \ref{stokes_kraus}, 
the transition in the asymptotic behavior of the parabolic 
cylinder function $D_\nu(-z)$ across the Stokes line 
$\arg z = \pi/4$, is the analytic signature of this 
POVM-to-PVM transition: the thermal regime $\sigma \to \infty$ 
corresponds to the $e^{-\frac{1}{4}z^2}$ asymptotic branch, 
and the localized regime $\sigma \to 0$ corresponds to the 
$e^{\frac{1}{4}z^2}$ branch. The present paper works entirely 
in the $\sigma \to 0$ PVM limit; \cite{player2025driving} 
works in the interior of the POVM family, with the Stokes 
line marking the boundary between thermal and non-thermal 
behavior.

\paragraph{The microstate.}
The result of both projections is an individually non-thermal 
microstate carrying a definite Rindler frequency $\omega$ and 
a definite transverse position. Thermality is not a property 
of any single microstate but of the ensemble; it emerges when 
one models the detector response within a wedge as a sum over 
all Rindler modes, as in the standard Unruh--DeWitt 
treatment~\cite{unruh1976notes, einstein1979general}, and is 
a consequence of that mode-averaging rather than a premise of 
the construction. The plane wave Unruh mode is what remains 
when neither coarse-graining is resolved; the thermal ensemble 
is what remains when neither is resolved and $\mathcal{L}$ is 
traced out; the individual non-thermal microstate is what the 
PVM projection onto the $\mathcal{P}$--$\mathcal{R}$ geodesic 
selects. The localization transition of 
Section~\ref{sec:loc_transition} is the geometric signature 
of this: the interpolation from the delocalized planar wave 
to the localized AS pp-wave traces the same POVM-to-PVM 
transition in the geometry of the horizon perturbation.
...
traces to the same algebraic symmetry. In the bilocal 
construction, the Bell state~\eqref{eq:microstate_vec} is the 
unique two-photon state invariant under the 
Bisognano--Wichmann modular conjugation $J$, which implements 
PCT between the two Rindler wedges and simultaneously flips 
$\lambda \to -\lambda$. In the zig-zag, PCT maps the 
right-helicity Weyl spinor $\psi_R$ to the left-helicity 
spinor $\psi_L$, precisely the same operation, now acting 
along the worldline rather than across a spatial horizon. The 
anticorrelated pair in each case is the PCT-invariant 
configuration of two massless helicity eigenstates; the 
bilocal Bell state and the Dirac 4-spinor are therefore the 
same object at different codimension: one lives on the 
$(1{+}1)$-dimensional worldline, the other on the 
$(2{+}1)$-dimensional horizon. The Lorentz algebra 
decomposition
\begin{equation}
\mathfrak{so}(3,1) \cong \mathfrak{su}(2) \times \mathfrak{su}(2)
\end{equation}
is the common algebraic statement: the two $\mathfrak{su}(2)$ 
factors label the two helicity sectors that PCT/$J$ exchanges 
in both cases.
...
irreducible representations. All physical results are 
...
\end{equation}
where $g$ is a coupling constant, $f_L$ and $f_R$ are 
normalised mode profiles supported on the left and right 
wedges respectively, and $\epsilon^i_{(\lambda)}$ are the 
...
between wedges. The Bell state~\eqref{eq:microstate_vec} is 
the unique two-photon state invariant under $J$: the 
anticorrelated helicity assignment and the symmetric 
superposition are not choices but consequences of requiring 
$J$-invariance. The bilocal 
source~\eqref{eq:bilocal_vec} is therefore the unique source 
consistent with the algebraic symmetry of the Rindler vacuum.
...
carries zero angular momentum, so the emitted pair must 
satisfy $\lambda_R + \lambda_L = 0$, forcing anticorrelated 
helicities; PCT then fixes the relative phase between the two 
terms, and the total angular momentum vanishes in both.
...
\end{equation}
where the two creation operator pairs correspond to the two 
PCT-related helicity assignments. The interaction term 
in~\eqref{eq:bilocal_lag} is a two-mode squeezing interaction 
at frequency $\omega$, now resolved by helicity and 
symmetrized over both assignments: it is the direct 
vector-field analogue of the scalar squeezing 
interaction~\cite{gerry2023introductory}, with the PCT 
symmetry between wedges made explicit. The photon pair 
carries total angular momentum zero in each term, consistent 
with emission from a scalar source at $\mathcal{P}$.
...
emitted quantum; rather, it determines the rate $\dot{N}$ at 
which the bilocal source must produce entangled pairs in order 
to fully\footnote{The bilocal source may be only part of the 
acceleration, so $\dot{N}$ is actually a lower bound.} 
implement that acceleration, with the total power 
$\dot{N}\hbar\omega$ matching the Unruh thermal flux. Each 
individual photon carries exactly one quantum of action 
...
same form in each sector. For helicity $\lambda$ at Rindler 
...
Unruh distribution at acceleration $a$, and the PCT symmetry 
between wedges is reflected in the superposition over both 
helicity assignments in the Bell state. The 
...
helicity-anticorrelated pairs at each Rindler frequency. The 
bilocal source, via the PVM projection of 
Section~\ref{sec:pvm_localization}, selects a single term 
from this sum at fixed $\omega$ and fixed helicity pair 
$(\lambda, -\lambda) = (+1,-1)$, yielding the single 
microstate
\begin{equation}
    |\Psi_{\omega}\rangle_M
    = \frac{1}{\sqrt{2}}\left(
...
to the two limits of the Kraus family~\eqref{eq:kraus}. 
...
\begin{itemize}
\item \textbf{Delocalized limit} ($\sigma \to \infty$, 
...
supertranslation mode is excited.

\item \textbf{Localized limit} ($\sigma \to 0$, POVM 
...
the Raychaudhuri equation applied to the null congruence 
depends only on the total null energy $\hbar\omega$, 
not on its transverse distribution;
the difference lies entirely in the 
spatial distribution of that increment and the corresponding 
BMS mode content. The delocalized regime describes the 
thermodynamic, ensemble-level face of the construction; the 
localized regime describes the individual microstate face and 
is where we will show $S_{GHY}|_{\mathcal{H}} = 
S_{\mathrm{matter}}$ holds event by event.
...
PVM limit of the Kraus family~\eqref{eq:kraus}: 
...
one-parameter Kraus family~\eqref{eq:kraus} 
...
We have proposed quantum measurement events as explicit models
of gravitational microstates
in the Rindler setting, 
...
directions. Matter and horizon are orthogonal information 
channels: matter the timelike substrate of 
emergent proper time, the horizon the spacelike 
surface through which PVM projections onto 
null geodesics generate emergent space, with 
the diagonal Kraus family~\eqref{eq:kraus} 
interpolating between the thermal ensemble 
and the individual non-thermal microstate.
This is evocative of ER=EPR at the 
