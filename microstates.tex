...
naturally constitutes a gravitational microstate 
...
Three well-established programs provide the 
grounding for the present construction, which 
supplies the missing physical process in each: 
quantum measurement. Jacobson~\cite{jacobson1995thermodynamics} derives the Einstein 
...
Consider the wedges in Figure \ref{paz} with distinguished points, right $\mathcal{R}$, left $\mathcal{L}$,
and past $\mathcal{P}$, at positions $(t,z) = (0,1)$, $(0,-1)$, and $(-1,0)$.
Each microstate, a projection of equation \eqref{eq:microstate_vec} upcoming,
...
Figure~\ref{fig:diagram}: state reduction
is not imposed from 
...
\subsection{A Source, Two Observers, and PVM Localization}
...
The two coarse-grainings are resolved by a single 
PVM projection defined by the physical 
source-detector geometry. The thermofield double 
state~\eqref{eq:vacuum_vec} is a sum over all 
Rindler frequencies $\omega$ and all transverse 
positions $\mathbf{x}_\perp$; specifying 
$\mathcal{P} = (-1,0)$ and $\mathcal{R} = (0,1)$ 
projects onto a single term in this sum 
simultaneously in both directions. The null 
geodesic connecting $\mathcal{P}$ to $\mathcal{R}$ 
has a unique frequency, determined by the proper 
distance and Rindler time between the two points, 
and a unique transverse position, determined by 
the spatial locations of the two points. A single 
PVM projection onto this geodesic therefore 
resolves the frequency coarse-graining and the 
transverse coarse-graining at once, selecting the 
microstate~\eqref{eq:microstate_vec} from the 
thermofield double. The plane wave Unruh mode is 
what remains when neither coarse-graining is 
resolved; the thermal ensemble is what remains 
when neither is resolved and $\mathcal{L}$ is 
traced out; the individual non-thermal microstate 
is what the PVM projection onto the 
$\mathcal{P}$--$\mathcal{R}$ geodesic selects. 
The localization transition of 
Section~\ref{sec:loc_transition} is the geometric 
signature of this projection: the interpolation 
from the delocalized planar wave to the localized 
AS pp-wave traces the same transition from 
unresolved to resolved transverse position that 
the PVM projection effects in the field theory.

The sharpness of this projection is controlled by 
the width $\sigma$ of the wavepacket shaping 
function, which acts as a diagonal Kraus operator
%
\begin{equation}
    K_\sigma = \int d^2\mathbf{x}_\perp\, 
    e^{-r^2/2\sigma^2}\,
    |\mathbf{x}_\perp\rangle
    \langle\mathbf{x}_\perp|,
\end{equation}
%
with complementary operator $K_\sigma^\perp$ 
satisfying $K_\sigma^\dagger K_\sigma + 
(K_\sigma^\perp)^\dagger K_\sigma^\perp = 
\mathbf{1}$ pointwise. This defines a POVM 
interpolating between two limits: $\sigma \to 
\infty$ gives $K_\sigma \to \mathbf{1}$, no 
projection, recovering the full thermal plane 
wave; $\sigma \to 0$ gives a sharp PVM projector 
onto the $\mathcal{P}$--$\mathcal{R}$ null 
geodesic, recovering the localized 
microstate~\eqref{eq:microstate_vec}. The Stokes 
phenomenon of~\cite{player2025driving}, the 
transition in the asymptotic behavior of the 
parabolic cylinder function $D_\nu(-z)$ across 
the Stokes line $\arg z = \pi/4$, is the analytic 
signature of this POVM-to-PVM transition: the 
thermal regime $\sigma \to \infty$ corresponds to 
the $e^{-\frac{1}{4}z^2}$ asymptotic branch, and 
the localized regime $\sigma \to 0$ corresponds 
to the $e^{\frac{1}{4}z^2}$ branch. The present 
paper works entirely in the $\sigma \to 0$ PVM 
limit; \cite{player2025driving} works in the 
interior of the POVM family.
...
The microstate~\eqref{eq:microstate_vec} admits two complementary geometric 
descriptions depending on the transverse localisation of the photon mode. 
The bilocal source fixes a definite Rindler frequency $\omega$ but imposes 
no constraint on transverse momentum. A mode with zero transverse momentum 
is uniform across the horizon cross-section; a wave packet of characteristic 
width $\sigma \sim c/\omega$ is localised within one wavelength of the photon 
axis. These two limits correspond to geometrically distinct pp-waves at the 
Rindler horizon, both analysed in Section~\ref{sec:ppwave}:

\begin{itemize}
\item \textbf{Delocalized limit} ($\sigma \to \infty$): the stress tensor is 
uniform across the horizon, producing a \emph{planar impulsive wave} with 
longitudinal shift $\Delta v \propto -r^2$. Exactly one BMS supertranslation 
mode is excited.

\item \textbf{Localized limit} ($\sigma \to 0$): the stress tensor concentrates 
near the beam axis, recovering the Aichelburg--Sexl pp-wave with logarithmic 
shift $\Delta v \propto -\ln r^2$. All BMS supertranslation modes are excited.
\end{itemize}

Both limits produce the same total horizon area increment 
$\Delta A = 8\pi G\hbar\omega$, since both carry total null energy $\hbar\omega$; 
the difference lies entirely in the spatial distribution of that increment 
and the corresponding BMS mode content. The delocalized regime describes 
the thermodynamic, ensemble-level face of the construction; the localized 
regime describes the individual microstate face and is where we will show 
$S_{GHY}|_{\mathcal{H}} = S_{\mathrm{matter}}$ holds event by event.
...
AS limit. The localization transition is therefore simultaneously a 
transition in the BMS mode content of the horizon perturbation, a 
transition in the spatial distribution of the area increment, and, 
via the correspondence established in~\cite{player2025driving}, the 
geometric counterpart of the QFT transition from delocalized thermal 
modes to localized non-thermal wave packets.
