...
\end{equation}
The state $|\Psi_\omega\rangle_M$ is obtained by projecting 
the Minkowski vacuum~\eqref{eq:vacuum_vec} onto the $n=1$ 
sector at fixed $\omega$ and fixed helicity pair. 
Multi-photon terms at $n \geq 2$ are present in the full 
vacuum decomposition but are not selected by the PVM 
projection, which by construction picks out the single-pair 
sector. The single-microstate description is therefore exact 
within the projected sector, with no weak-coupling assumption 
required.
