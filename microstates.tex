...
Motivated by this, we construct a helicity-anticorrelated Bell state, emitted from a common causal past, that drives the Unruh response in both Rindler wedges of flat Minkowski space.
...
The elimination of
temperature in this section conceptually and physically leads the way to the
next Section \ref{sec:microstates}, where we construct concrete
non-thermal microstates that
drive the Unruh response, and the later sections that study our microstate
with an information theoretic langauge
and connect it to literature.
It allows us to work in the non-thermal 
global Minkowski description, where thermality is a derived consequence 
of restricting to one wedge~\cite{fulling1973nonuniqueness, bisognano1975duality}
...
family equation~\eqref{eq:kraus} upcoming.
...
This distinction is operationally sharp~\cite{spekkens2005contextuality, 
harrigan2010einstein}: \textit{mode shaping} is a preparation that 
modifies the physical field; \textit{PVM projection} is a measurement 
that conditions the observer's description on causal data without 
touching the ontic state. The Gaussian width $\sigma$ of the Kraus 
...
Since the two widths $\sigma$ and $\sigma_\perp$ control 
physically distinct aspects of the construction they 
...
The BMS mode content excited by each transverse profile 
...
transverse localisation of the photon mode, controlled 
by the transverse Kraus family
\begin{equation}
...
\label{eq:kraus}
\end{equation}
the direct transverse analogue of~\eqref{eq:kraus_longitudinal}.
...
longitudinal shift $\Delta v \propto -r^2$, which is a 
supertranslation in the BMS sense.
...
logarithmic shift $\Delta v \propto -\ln r^2$.
\end{itemize}

Both limits produce the same total horizon area increment 
$\Delta A = 8\pi G\hbar\omega$ (equation~\eqref{eq:deltaA_result}), 
since the Raychaudhuri equation depends only on the total 
null energy $\int T_{uu}\,du\,d^2x_\perp = \hbar\omega$, 
fixed by energy conservation independently of the 
transverse profile. The difference between the limits 
lies in the spatial distribution of that increment and 
the BMS mode content excited by each profile.
...
A communication is a horizon-crossing particle\footnote{For our microstate this is a photon, however, it could be more general.}
...
The area increment $\Delta$ is 
independent of the transverse localization of the photon. 
In the delocalized limit the stress tensor is uniform 
across the horizon and produces a planar shift 
$\Delta v \propto -r^2$; in the localized limit it 
concentrates near the beam axis and produces the AS 
logarithmic shift $\Delta v \propto -\ln r^2$. Both 
are supertranslations in the BMS sense and both give 
the same area increment, as the Raychaudhuri 
calculation below shows. The one-parameter family of 
Gaussian transverse profiles interpolating between 
these two limits, parametrized by width $\sigma$, is 
the geometric counterpart of the POVM-to-PVM transition 
of Section~\ref{sec:pvm_localization}: the spatial 
distribution of the area increment sharpens from 
uniform to concentrated as $\sigma \to 0$, while 
the total $\Delta A$ remains fixed by energy 
...
but applied here to a single microstate rather than 
a thermodynamic or ensemble average.
...
event: in the delocalized limit the photon produces a planar 
supertranslation displacing both wedges uniformly, while 
in the localized limit each individual measurement event 
produces a single AS longitudinal shift concentrated near the photon 
...
In the present flat-space construction the ER bridge is replaced by 
the AS longitudinal shift: each measurement event displaces the two 
wedges apart in opposite null directions, creating the coordinate 
space between them that the ER bridge represents in the AdS setting. 
The entanglement is not a property of the geometry; it is the 
\textit{cause} of it, event by event.
...
a bit that now converts a polarization
...
records. The construction does not derive spatial emergence 
in full generality; what it shows is that the vocabulary 
...
have a natural analogue along a timelike worldline. Penrose's 
zig-zag construction~\cite{penrose2006road} represents 
a massive fermion as two massless Weyl spinors of 
opposite helicity, coupled at each vertex by the Higgs 
field and alternating in a zig-zag pattern; we term 
this the tick-tock mechanism to emphasise the 
alternating rhythm of communications rather than the 
geometry of the path. The parallel with the bilocal 
construction is structural: the Higgs field plays the 
role of $\mathcal{P}$, the two Weyl spinors play the 
role of the two photons propagating to $\mathcal{R}$ 
and $\mathcal{L}$, and each vertex is a communication unit
depositing one quantum of action and producing one 
sample of proper time rather than one increment of 
horizon area. We present this as a suggestive analogy 
