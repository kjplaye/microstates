...
This paper adopts an epistemic reading of quantum 
state reduction throughout. The wavefunction is 
treated as an agent's belief state, and collapse 
is a Bayesian update for a given observer rather 
than an objective physical event. This is the 
precise sense in which the present approach is a 
refactoring of the Di\'osi--Penrose 
program~\cite{penrose1996gravity,diosi1987universal}: 
that program asks what objective physical mechanism 
triggers collapse; the present paper takes a 
different path, treating state reduction as 
inherently observer-relative while remaining 
agnostic about whether an objective collapse 
mechanism also exists. Observer-relative microstate 
records remain intersubjectively consistent because 
they share a common causal ancestry, as our 
construction makes explicit. The dashed arrow of 
Figure~\ref{fig:diagram} is dashed not because the 
connection is speculative but because it presupposes 
an ontic reading of state reduction that this paper 
does not explore; the solid arrows are the path we 
follow.
