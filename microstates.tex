...
a handoff event as an elementary unit transferring quanta of
action and information across that horizon, 
...
Penrose zig-zag, where
the bilocal Bell state and the Dirac 4-spinor 
realize the same PCT-invariant helicity structure 
in spacelike and timelike separations respectively. (Sections~\ref{sec:types}--\ref{sec:zigzag}).
...
the phase $e^{iS/\hbar}$ completes one cycle, and 
$\hbar$ is the minimal unit of action that can be 
transferred in a single interaction. The regime in 
which this description operates for our microstate, and its natural 
boundaries, are developed in Section~\ref{sec:eft}.
...
sample of the phase $e^{iS/\hbar}$ in the path 
integral and we use the information-theoretic term \emph{sample} 
throughout to denote a single zero crossing. The term 
\emph{event} may be substituted throughout. The 
sampling resolution is limited by the Heisenberg 
uncertainty $\Delta E \Delta t \sim \hbar$, which 
plays the role of the Nyquist limit for the matter 
channel: sub-Compton phase structure cannot be 
resolved without energy sufficient to leave the 
single-particle regime~\cite{millette2011heisenberg}.
...
interaction that transfers action into matter-memory 
...
PCT-invariant configuration, whether the separation 
between
