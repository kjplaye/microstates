...
% TODO 1.7 [NEW]: Sharpen the Landauer-for-acquisition argument.
% Bennett's result is that measurement can in principle be
% performed reversibly; it is erasure that necessarily costs
% energy. Section 2 applies the Landauer bound to acquisition on
% the grounds that "registering a measurement outcome is
% physically equivalent to information being written into the
% detector's state." This conflation is the most likely referee
% objection in the paper. Either (a) provide a physical argument
% specific to this setting for why acquisition is irreversible
% here — e.g., that the photon is absorbed and the record
% persists in a degree of freedom that cannot be coherently
% reversed without a separate erasure step — or (b) explicitly
% label Section 2 as a heuristic derivation, defer the rigorous
% Landauer accounting entirely to Section 7, and add a forward
% reference at the point of first use.
...
bound, where the elimination is both algebraic and conceptual, 
reflecting that temperature is a property of the coarse-grained 
Rindler description absent from the underlying Minkowski 
microstates, and reproduce standard black hole thermodynamics 
