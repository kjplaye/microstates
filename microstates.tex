...
%TODO 2.3 [NEW — Section 3]: Specify the order of the remainder in equation (16).The microstate vector ∣Ψω⟩M|\Psi_\omega\rangle_M∣Ψω​⟩M​ is written with + O(higher order)+\,\mathcal{O}(\text{higher order})+O(higher order) but the expansion parameter is not named. Higher order in the coupling λ\lambdaλ? In the squeezing parameter tanh⁡θω\tanh\theta_\omega tanhθω​? In the source amplitude? State explicitly, since the validity of treating this as a single-microstate depends on the suppression of multi-pair terms, and a referee will ask.
...
% clearly.
%TODO 3.1 [NEW — Section 5.3]: Fix the dimensional error in the entropy increment line.The expression ΔS=2π ΔEℏω/ℏ\Delta S = \frac{2\pi\,\Delta E}{\hbar\omega/\hbar}ΔS=ℏω/ℏ2πΔE​ has units of energy in the denominator (ℏω/ℏ=ω\hbar\omega/\hbar = \omega ℏω/ℏ=ω, a frequency). The intended cancellation is ΔE/ℏω=1\Delta E / \hbar\omega = 1 ΔE/ℏω=1, leaving ΔS=2π\Delta S = 2\pi ΔS=2π, but the intermediate step as typeset is wrong. Rewrite as ΔS=(2π/ℏω)⋅ℏω=2π\Delta S = (2\pi/\hbar\omega)\cdot\hbar\omega = 2\pi ΔS=(2π/ℏω)⋅ℏω=2π or simply state the substitution ΔE=ℏω\Delta E = \hbar\omega ΔE=ℏω explicitly before the line.
...
% ----------------------------------------------------------------
%TODO 6.2 [NEW — Section 7]: Justify the identification kBTH=ℏωk_B T_H = \hbar\omegakB​TH​=ℏω.Equation (47) identifies the hot reservoir temperature with the energy of a single photon via radiation pressure, but this step is not derived — it is essentially the Planck relation applied to one mode. The difficulty is that the paper operates in the non-thermal Minkowski description where the photon has a definite frequency ω\omegaω and is not thermal. A referee will ask why this single-frequency photon sets the Unruh temperature of the horizon rather than being one mode in a thermal distribution. Either derive this from the Bogoliubov transformation (the Rindler temperature is related to ω\omega ω and aaa via tanh⁡θω\tanh\theta_\omegatanhθω​, not directly to ℏω\hbar\omegaℏω) or explicitly flag this as an ansatz with the same status as the Bekenstein–Hawking ansatz in Section 5.3.
...
relate spacetime dynamics to horizon entropy but leave unspecified 
the microscopic carriers of individual entropy increments. 
We identify these with quantum measurement events, realized 
explicitly as helicity-anticorrelated Bell states of photons 
sourced by a bilocal driving term in the Rindler setting. 
The central result is an event-by-event equality 
...
horizon geometry counts measurement events in geometric language. 
From a single temperature-eliminated Unruh--Landauer bound, 
standard black hole thermodynamics follows from Newton's laws. 
These microstates are grounded in a concrete information-theoretic 
picture of matter: massive bodies are timelike channels sampling 
internal degrees of freedom at the Bremermann--Nyquist rate 
$mc^2/h$, massless quanta are pure carriers between such channels, 
and horizons are the spacelike surfaces at which handoff events 
occur, with proper time emerging as accumulated quanta of action 
along the worldline. This provides a framework 
consistent with Wheeler's it-from-bit, in which the physical 
record of each binary measurement outcome is written into matter, 
and spacetime geometry is a derived property of matter's 
...
correlations are sampled. This section develops the 
channel picture and derives three results that 
Section~\ref{sec:ppwave} realises geometrically: the 
proportionality of the Bremermann and Bekenstein bounds 
(Section~\ref{sec:horizon_channel}), a formal definition 
...
The Bekenstein and Bremermann bounds are not merely 
analogous: they count the same physical event from 
orthogonal directions, related by the Unruh--Landauer 
relation of Section~\ref{sec:heuristics}. To see this 
explicitly, work in natural units $\hbar = c = 1$ and 
express frequencies in units of the Rindler acceleration 
$a$, so that $\Omega \equiv \omega/a$ is dimensionless. 
For a single photon of dimensionless Rindler frequency 
$\Omega$ crossing the horizon, the Bremermann count 
is the number of new resolution elements added to the 
receiving timelike channel per Rindler period 
$\tau = 2\pi/a$:
%
\begin{equation}
    \Delta f \cdot \tau 
...
\end{equation}
%
The Bekenstein count of the same event is the spatial 
entropy increment. By the Raychaudhuri calculation of 
Section~\ref{sec:ppwave}, the horizon area increments 
by $\Delta A = 8\pi G\omega = 8\pi G a\Omega$, giving
%
\begin{equation}
    \Delta H_{\mathrm{Bek}} 
    = \frac{\Delta A}{4\ell_P^2}
    = \frac{8\pi G a\Omega}{4G}
    = 2\pi\Omega.
    \label{eq:bekenstein_count}
\end{equation}
%
The two counts satisfy
%
\begin{equation}
    \boxed{\Delta H_{\mathrm{Bek}} 
    = 2\pi \cdot (\Delta f \cdot \tau),}
    \label{eq:brem_bek_equiv}
\end{equation}
%
with the factor $2\pi$ being the Rindler normalisation---the 
same factor appearing in the Unruh temperature 
$T = \hbar a / 2\pi k_B c$ and tracing to the 
analytic continuation between Minkowski and Rindler 
coordinates. At the fundamental Rindler mode $\Omega = 1$ 
(one cycle per Rindler period), the Bremermann count 
gives one new resolution element and the Bekenstein 
count gives $\Delta H = 2\pi$ nats, consistent with 
the direct calculation of Section~\ref{sec:ppwave}. 
The equality $S_{GHY}|_{\mathcal{H}} = 
S_{\mathrm{matter}}$ of Section~\ref{sec:ppwave} is 
the geometric proof of~\eqref{eq:brem_bek_equiv}: 
counting Planck-area patches spatially and counting 
action quanta temporally give the same result up to 
the universal Rindler factor $2\pi$.
...
for the Rindler case, and substituting $\Delta E = \hbar\omega$, yields a corresponding entropy 
increment
\begin{equation}
\Delta H = \frac{\Delta A}{4\ell_P^2} 
         = \frac{2\pi\,\Delta E}{\hbar\omega}
         = 2\pi,
\end{equation}
