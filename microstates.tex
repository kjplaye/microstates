...
The two geometric regimes of Section~\ref{sec:two_regimes} carry complementary 
entanglement interpretations.
In the delocalized limit, the planar impulsive 
...
argument~\cite{van2010building}: the delocalized photon produces a 
global deformation of the geometry connecting the two wedges via 
a BMS supertranslation at the horizon
...
concrete physical process: each measurement event 
produces an AS longitudinal shift that is a 
supertranslation in the BMS sense.
...
for future work. The relation to the Ryu--Takayanagi 
formula~\cite{ryu2006holographic} is a natural question 
in this context: the present result holds in flat Minkowski 
space on a Rindler horizon rather than on a minimal surface 
in AdS, is an equality of actions rather than entropies, 
and holds event by event rather than as a thermodynamic 
average. Whether $S_{GHY}|_{\mathcal{H}} = 
S_{\mathrm{matter}}$ is the flat-space Rindler limit of 
Ryu--Takayanagi, or an independent derivation of the same 
underlying holographic principle, we leave open.

The classical focusing theorem suffices for the pure 
microstates treated here; the natural extension beyond 
pure microstates is the Quantum Null Energy 
Condition~\cite{bousso2016quantum, ceyhan2020recovering}, 
which replaces the null energy condition with a bound 
involving the second derivative of von Neumann entropy 
along the null generators. We leave its precise role 
at the boundary of EFT validity for future work.
