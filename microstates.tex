...
increment for a given Rindler observer. The physical 
events (the photon pair, the AS pp-wave, the area 
increment $\Delta A = 8\pi G\hbar\omega$) are 
Lorentz-covariant; what is observer-relative is their 
decomposition into Rindler modes, exactly as a Lorentz 
boost changes a photon's frequency without changing 
the physics. This is not the statistical recovery of 
Lorentz invariance familiar from discrete spacetime 
approaches~\cite{wheeler1955geons,bombelli1987space}; 
it is covariance at the level of individual events, 
with the matter sector ($\hbar$, photon helicity) 
carrying the covariance and geometry inheriting it 
through the Raychaudhuri equation.
...
raised by Wigner's 
friend~\cite{brukner2018nogofacts} and addressed in 
the relational~\cite{rovelli1996relational} and 
QBist~\cite{fuchs2014qbism} programmes. Standard QFT 
