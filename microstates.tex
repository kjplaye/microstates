...
single physical event, a communication between matter 
...
physical event: a communication event that transfers one quantum 
...
present construction can be read as a microstate-level realisation of the Bousso bound, with each communication event contributing one Planck-area patch to the light-sheet.
...
We develop a language that will provide a way of thinking about our microstate as a building block for emergent spacetime. We discuss matter action $S_{\mathrm{matter}}$ as a measurement of proper time; $\mathcal{P}$ and $\mathcal{R}$ are massive observers that accumulate this action. Our microstate transfers a portion, $\hbar$, of this action across the horizon, a communication, which we
study as the Gibbons--Hawking--York boundary term in the next Section \ref{sec:microstate_action}. Then we find that a bit of space, $\mathcal{R}$ vs. $\mathcal{L}$, emerges naturally in Section \ref{sec:erepr}. In this way, space and time can be measured and quantized as properties of matter and interactions. Our unifying language is going to be that of classic information theory, channels.


Specifically, we propose that matter is the fundamental substrate of 
spacetime information processing by reinterpreting 
quantum mechanics in an information-theoretic way:
proper time emerges from discrete action quanta 
accumulated along worldlines, spatial geometry emerges from 
communication events at horizons, and the Planck area $\ell_P^2$ 
is the conversion factor between the two. Discreteness lives
in the matter 
sector via $\hbar$, and geometry inherits it through the 
covariant Raychaudhuri equation.
The following subsections develop this in stages:

\begin{enumerate}
    \item \textbf{Matter as a timelike channel} 
    (Section~\ref{sec:channel}): a massive particle 
    carries phase at the Compton frequency, setting 
    its clock rate and maximum information throughput.
    \item \textbf{The horizon as a spacelike channel} 
    (Section~\ref{sec:horizon_channel}): the horizon 
      is a separating surface through which two timelike matter channels
      communicate with $\ell_P^2$     as the conversion factor.
    \item \textbf{The communication event} 
      (Section~\ref{sec:communication}): The communication, an elementary
      massless carrier, 
    transfers action and information between matter-memories.
\end{enumerate}
We then discuss particle types and types of interactions (Section~\ref{sec:types}), and finish up with a way to understand Dirac's electron as a Compton clock (Section~\ref{sec:zigzag}) connecting our framework to the Standard Model via Penrose's zig-zag.
...
We propose that the matter substrate can be assigned a 
channel capacity in the Shannon sense: for a system of mass $m$ 
...
this is a proposed organizing quantity, not a derived bound 
on the same footing as equations~\eqref{eq:UL} 
or~\eqref{eq:deltaA_result}. 
...
The channel capacity~\eqref{eq:channel_capacity} is offered as 
...
and that crossing is the communication event, converting 
...
\subsection{The Communication Event}
\label{sec:communication}

The photon crossing the horizon is a communication between 
the timelike matter channels of $\mathcal{R}$ and 
$\mathcal{L}$, mediated by the spacelike horizon channel.
To that end we define:

\begin{quote}
\textbf{Definition (Communication Event).}\ 
A \emph{communication event} is a horizon-crossing 
interaction that transfers action and information
between matter-memories.
\end{quote}
...
Each communication event has exactly two irreducible 
...
sums $\hbar\omega$ over communication events because that 
...
substrates as communication carriers, while massive systems 
...
information carrier effecting a communication between the 
...
a persistent record survives. The ideal mirror
is a limiting 
...
The communication event of Section~\ref{sec:communication} has a 
natural analogue along a timelike worldline. Penrose's 
...
alternating rhythm of communication events rather than the 
...
and $\mathcal{L}$, and each vertex is a communication event 
...
between communication events. Each vertex is a communication event: 
...
discrete sequence of communication events, each acting as a PVM 
...
matter substrate and the communication event are the microscopic 
...
with each communication event realising one entropy increment of 
...
projections, communication events, matter-memory, is the 
...
of the communication definition of Section~\ref{sec:communication} 
...
about the communication event itself, not about the subsequent 
...
the communication event of Section~\ref{sec:communication}. The Raychaudhuri 
...
along worldlines, space is the record of communication events 
...
communication events, matter-memory, PVM projections, the 
