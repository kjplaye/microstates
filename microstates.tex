...
\abstract{
We propose that quantum measurement events are the 
microscopic carriers of individual entropy increments 
in thermodynamic approaches to gravity
and develop the information-theoretic vocabulary to make
that connection precise.
Matter is the 
timelike substrate of emergent proper time; the horizon 
is the spacelike surface through which handoff events 
generate emergent space; and the Planck area $\ell_P^2$ 
is the conversion factor between the two descriptions. 
...
information acquisition. A helicity-anticorrelated Bell 
state in flat Minkowski space serves as the explicit 
microstate: a bilocal source in the past emits an 
entangled photon pair whose absorption by two Rindler 
observers produces a discrete horizon area increment 
$\Delta A = 8\pi G\hbar\omega$ and a spatial bit --- 
a classical record that encodes not a polarization state 
but a location in space. Space, in this construction, 
is a consequence of helicity. The construction makes 
contact with Jacobson's thermodynamic derivation of the 
Einstein equations, Van Raamsdonk's 
entanglement-generated geometry, and the BMS 
supertranslation structure at horizons.}
...
% can naturally result in a gravitational microstate 
% by developing an information-theoretic language 
% of \emph{matter-memory}, \emph{handoff events}, 
...
% geometry, the BMS supertranslation structure at horizons.}
...
We will show how the answer can be quantum measurement by developing an information-theoretic language that connects ``thrust'' microstates, that can drive the Unruh effect \cite{player2025driving}, to the general quantum gravity program. We do this philosophically and with an explicit microstate construction which we then connect to current literature.

Before we dig in though, we first briefly outline our high level philosophical argument about how quantum measurement naturally connects to gravity. The general setup is pictured in the commutative\footnote{This refactors the Di\'{o}si--Penrose postulate. } diagram in Figure \ref{fig:diagram}, which we will walk through from the top right clockwise around the bottom to the top left. Quantum state reduction always happens due to an observation. Any observation is mediated by a physical interaction: information transfer necessarily involves an exchange of energy and momentum between the observed system and the detector, a discrete impulse. And then the equivalence principle implies that this impulse is locally indistinguishable from the effect of a gravitational field.
...
single physical event,  one handoff between matter 
...
is Proposition~3 of that paper: the overlap between a 
