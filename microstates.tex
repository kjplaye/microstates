...
clock rate of any physical system with bounded energy.

We associate a discrete event with one zero crossing 
...
matter channel~\cite{millette2011heisenberg}: it would 
require additional energy $\Delta E > mc^2$ to probe 
the sub-Compton phase.

The signal being sampled at the Bremermann rate is 
the internal quantum state of the particle, its 
spin, helicity, and other quantum numbers, and the 
dynamic range is $\log_2$ of the dimension of the 
internal Hilbert space. Any signal written into the 
matter channel must have energy exceeding the ambient 
thermal noise floor to constitute a distinguishable 
record: this is Landauer's 
argument~\cite{landauer1961irreversibility}, applied 
here not to erasure but to registration. In the 
Rindler frame the noise floor is set by the Unruh 
temperature $k_BT = \hbar a/2\pi c$, so the minimum 
energy per registered nat is precisely the 
Unruh--Landauer bound~\eqref{eq:UL}. The matter 
channel capacity in an environment with acceleration 
$a$ is therefore:

\begin{definition}
For a system of mass $m$ with internal Hilbert space 
dimension $d$ and ambient acceleration $a$, the 
matter channel capacity is
\begin{equation}
    C(a) = \underbrace{\frac{mc^2}{h}}_{\text{bandwidth}} 
        \times 
        \underbrace{\log_2(d)}_{\text{dynamic range}}
        \quad \text{(bits per second),}
    \label{eq:channel_capacity}
\end{equation}
where the bandwidth is the Bremermann sampling rate, 
the dynamic range is the information content per 
sample, and each sample is distinguishable from the 
Unruh noise floor provided the signal energy satisfies 
the Unruh--Landauer bound~\eqref{eq:UL}. At $a = 0$ 
the noise floor is the ambient temperature $k_BT$ 
and the bound reduces to the standard Landauer 
condition.
\end{definition}

Both bandwidth and dynamic range are extensive 
under composition:
\begin{equation}
    \frac{mc^2}{\hbar} 
...
matter that compose naturally under union. This 
extensivity is independent of the noise floor and 
holds for any value of $a$.

\paragraph{Saturation at the horizon.}
The Unruh--Landauer bound~\eqref{eq:UL} is saturated 
when the photon energy equals the minimum cost of 
registering one bit against the Unruh noise floor. 
Setting $\Delta H = \ln 2$ (one bit) and saturating 
the inequality:
\begin{equation}
    \hbar\omega_{\mathrm{sat}} 
    = \frac{\hbar a}{2\pi c} \ln 2,
    \qquad
    \omega_{\mathrm{sat}} = \frac{a \ln 2}{2\pi c}.
\end{equation}
A photon below $\omega_{\mathrm{sat}}$ cannot 
constitute a complete one-bit measurement record 
against the Unruh noise floor; a photon above 
$\omega_{\mathrm{sat}}$ deposits more than the 
minimum required. The horizon condition 
$a = c^2/R$ then gives
\begin{equation}
    \lambda_{\mathrm{sat}} 
    = \frac{2\pi c}{\omega_{\mathrm{sat}}} 
    = \frac{4\pi^2}{\ln 2} \cdot \frac{c^2}{a}
    \sim d_{\mathcal{H}},
\end{equation}
recovering the saturation wavelength of 
Section~\ref{sec:UL} directly from the noise-floor 
condition. The horizon is therefore the unique surface 
at which the minimum energy cost of information 
registration (Landauer) and the Bremermann sampling 
rate coincide in a single-bit event: the matter 
channel saturates exactly at the Rindler horizon, 
and the Planck area $\ell_P^2$ is the conversion 
factor between the geometric and informational 
descriptions of that saturation.
For composite systems the internal Hilbert 
space includes additional degrees of freedom 
beyond spin and helicity; whether the full 
$d$-dimensional space is accessed at each 
Bremermann sample depends on the dynamics, and 
in general $C(a)$ is an upper bound rather than 
a guaranteed saturation rate. The zig-zag 
construction of Appendix~\ref{sec:zigzag} 
provides a concrete case where saturation is 
achieved: the Higgs vertex forces a helicity 
flip at each Compton time, accessing both 
dimensions of the Weyl spinor Hilbert space 
at the Bremermann rate. Whether this extends 
to more complex internal spaces is left to 
future work.
